...
type expr =
  ...
  | StructInit of id_expr * (string option * expr) list (* Point { .x = 1, .y = 2 } *)
...
and stmt =
  ...
  | Auto of string * expr
  | Assign of expr * assign_op * expr               (* x = 10; *)
  | Expr of expr                        (* func(); *)
  | Block of stmt list                  (* { ... } *)
...
type func_param  =
    { p_type: typ; p_name: string ; default_value: expr option}
[@@deriving show]
